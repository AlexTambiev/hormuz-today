...
const STATUS_TIMEZONE = process.env.STATUS_TIMEZONE || "Europe/London";
...
async function fetchFeed(source) {
  ...
  try {
    ...
    return {
      source: source.name,
      ok: true,
      count: parseFeed(xml, source.name).length,
      items: parseFeed(xml, source.name),
    };
  } finally {
...
function verdictFromEvidence(items, sourceHealth, errors = []) {
  const closed = items
...
async function refreshStatus() {
  ...
  const verdict = verdictFromEvidence(items, sourceHealth, errors);
...
async function getStatusForToday() {
